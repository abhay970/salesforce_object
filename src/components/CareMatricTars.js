...
import CareMatricTarNew from "./CareMatricTarNew";

const CareMatricTars = () => {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        const response = await axios.get("/carematrictarget");
        ...
        <tbody>
          {leads.map((lead, index) => (
            <tr key={lead.id}>
              ...
              <td>{lead.id}</td>
              <td>
                <Link to={`/carematrictarget/${lead.id}`}>{lead.name}</Link>
              </td>
              <td>{lead.unitcode}</td>
              {/* <td>{lead.unitcode}</td>
               */}
      ...
      {openModal && (
        <div className="modal">
          <div className="overlay">
            ...
            <CareMatricTarNew toggleOpenModal={toggleOpenModal} />
          </div>
        </div>
      )}
    </div>
