...
import CodeSetNew from "./CodeSetNew";
...
const CodeSets = () => {
  ...
  useEffect(() => {
    const fetchLeads = async () => {
      try {
        const response = await axios.get("/codesets");
        ...
        <tbody>
          {leads.map((lead, index) => (
            <tr key={lead.id}>
              ...
              <td>{lead.id}</td>
              <td>
                <Link to={`/codesets/${lead.id}`}>{lead.name}</Link>
              </td>
      ...
      {openModal && (
        <div className="modal">
          <div className="overlay">
            ...
            <CodeSetNew toggleOpenModal={toggleOpenModal} />
          </div>
        </div>
      )}
    </div>
